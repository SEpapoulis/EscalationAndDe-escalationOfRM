...
#Lets gather all organism information
RMprots=Fasta('data/search_results/ElementsFound.fasta')
noRMorgs = set(load_simpletxt("data/search_results/No_element_orgs.txt"))
failed = set(load_simpletxt("data/search_results/failed.txt"))
genome_stats = pd.read_csv("data/search_results/orgid_dat.csv",header=None)
...
df=df[['orgid','genus','phyla','GenomeSize(bp)','NumContigs','mT1','rT1','sT1','rmT1',
            ...
            'pr','prm','rmT?','pT2G']]    
#Saving Data
df.to_csv('data/RMdat.csv',index=False)
pTIIG.write_file('data/pTIIG/pTIIG.fasta')
RMprotclass=pd.DataFrame(RMprotclass)
RMprotclass.columns = ['orgid', 'accession','length','type','cluster','rmT1','rmT2','rmT3','rmT?']
RMprotclass.to_csv('data/RMprotclass.csv',index=False)
